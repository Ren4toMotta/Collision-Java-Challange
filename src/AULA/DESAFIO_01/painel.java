...
public class painel extends JPanel implements MouseListener, KeyListener {
	...
	public void addMuro() {
		muros.add(new obstaculos(80,0,70,200));
		muros.add(new obstaculos(350,200,120,120));
		muros.add(new obstaculos(430,445,300,50));
		muros.add(new obstaculos(600,0,50,360));
		muros.add(new obstaculos(80,320,70,180));
		
	}
	
	public void addJogador() {
		jogadores.add(new player("Amendoim", 'O', this.getWidth(),this.getHeight(), muros));
		jogadores.add(new player("Batatinha", 'R', this.getWidth(), this.getHeight(), muros));
		jogadores.add(new player("Pastel", 'P', this.getWidth(),this.getHeight(), muros));
...
// =============================================================================================================
	...
	public void mouseClicked(MouseEvent e) {
		if(e.getButton() == MouseEvent.BUTTON1) {
			for(player j : jogadores) {
				boolean ativado = j.veriAtivPlayer(e.getX(), e.getY());
				if(ativado) {
					j.ativarplayer(true);
				} else {
	...
	public void keyPressed(KeyEvent e) {
		...
		for(player j : jogadores) {
			if(direcao != 'A' && j.estaAtivo()) {
				boolean colidiu = j.verifColisoes(direcao, 5, muros, jogadores);
				if(!colidiu){
					this.repaint();					
				}
			}
